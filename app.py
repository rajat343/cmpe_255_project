...
def extract_images_from_pdf(pdf_path: Path, save_dir: Path, pdf_hash: str, db_path: Path) -> List[str]:
    ...
    for page_index in range(len(doc)):
        for img_index, img in enumerate(doc[page_index].get_images(full=True)):
            ...
            # BLIP caption
            blip_caption = describe_image_blip(str(image_path))
            ...
            st.session_state.image_ocr_docs.append(Document(
                ...
                # BLIP caption document
            st.session_state.image_ocr_docs.append(Document(
                page_content=blip_caption,
                metadata={
                    "type": "caption",
                    "source_type": "image",
                    "page": page_index + 1,
...
def extract_text_from_image(image_path: str) -> str:
    ...
    if not result or not result[0]:
        print(f"⚠️ No OCR result for image: {image_path}")
    ...
    with st.spinner("Processing your query…"):
        if q in st.session_state.qa_cache:
            ...
            for d in docs:
                ...
                file = meta.get("image_file", meta.get("source", "Unknown"))

                if doc_type == "ocr":
                    sources.append(f"🖼️ OCR Text (Page {page}) – {file}")
                elif doc_type == "caption":
                    sources.append(f"🧠 BLIP Caption (Page {page}) – {file}")
                elif doc_type == "text":
                    sources.append(f"📄 PDF Text (Page {page})")
                else:
                    sources.append(f"📄 Unknown Source (Page {page}) – {file}")
